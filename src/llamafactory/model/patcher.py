...
def patch_valuehead_model(model: "AutoModelForCausalLMWithValueHead") -> None:
    ...
    def get_rope_index(self: "AutoModelForCausalLMWithValueHead", *args, **kwargs):
        """Proxy method to access get_rope_index from the underlying pretrained model."""
        if hasattr(self.pretrained_model, "get_rope_index"):
            return self.pretrained_model.get_rope_index(*args, **kwargs)
        elif hasattr(self.pretrained_model, "model") and hasattr(self.pretrained_model.model, "get_rope_index"):
            return self.pretrained_model.model.get_rope_index(*args, **kwargs)
        else:
            raise AttributeError(f"Neither pretrained_model nor pretrained_model.model has get_rope_index method")
    ...
    # Add get_rope_index method if the underlying model has it
    if (hasattr(model.pretrained_model, "get_rope_index") or 
        (hasattr(model.pretrained_model, "model") and hasattr(model.pretrained_model.model, "get_rope_index"))):
        setattr(model, "get_rope_index", MethodType(get_rope_index, model))
