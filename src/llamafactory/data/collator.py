...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __post_init__(self):
        ...
        # Handle AutoModelForCausalLMWithValueHead
        if isinstance(self.model, AutoModelForCausalLMWithValueHead):
            actual_model = self.model.pretrained_model
            if isinstance(actual_model, PeftModel):
                actual_model = actual_model.base_model.model
        else:
            actual_model = self.model

        if actual_model is not None and hasattr(actual_model, "get_rope_index"):  # for qwen2vl mrope
            self.get_rope_func = actual_model.get_rope_index  # transformers < 4.52.0 or qwen2.5 omni
        elif actual_model is not None and hasattr(actual_model, "model") and hasattr(actual_model.model, "get_rope_index"):
            self.get_rope_func = actual_model.model.get_rope_index  # transformers >= 4.52.0
        elif self.model is not None and hasattr(self.model, "get_rope_index"):  # for patched valuehead models
            self.get_rope_func = self.model.get_rope_index
        else:
            self.get_rope_func = None
        # assert self.get_rope_func is not None, "get_rope_index function is required for some models, e.g., qwen2vl."
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        if self.get_rope_func is not None:
            ...
            if isinstance(self.model, AutoModelForCausalLMWithValueHead):
                model_config = self.model.pretrained_model.config
            else:
            ...
            if getattr(model_config, "model_type", None) == "qwen2_5_omni_thinker":  # for qwen2.5 omni
        ...
        # Get the actual model config for validation
        if isinstance(self.model, AutoModelForCausalLMWithValueHead):
            model_config = self.model.pretrained_model.config
        else:
            model_config = getattr(self.model, 'config', None)
        ...
        if (
            ...
            and getattr(model_config, "model_type", None)
            in ["glm4v", "Keye", "qwen2_vl", "qwen2_5_vl", "qwen2_5_omni_thinker"]
            and ("position_ids" not in features or features["position_ids"].dim() != 3)
        ):
            raise ValueError(f"{model_config.model_type} requires 3D position ids for mrope.")
...
class PairwiseDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for key in ("chosen", "rejected"):
            for feature in features:
                # Handle image assignment for pairwise data
                images_for_sample = feature["images"]
                image_count = feature['image_count']
                if images_for_sample:
                    # If there are image_count['chosen'] + image_count['rejected'] images, assign n to chosen and n to rejected
                    
                    if key == "chosen":
                        images_for_sample = images_for_sample[:image_count['chosen']]
                    else:  # rejected
                        images_for_sample = images_for_sample[image_count['chosen']:]
